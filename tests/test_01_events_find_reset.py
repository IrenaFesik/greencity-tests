...
if __name__ == "__main__":
    ...
    events_find_xpath = "//p[contains(text(), 'Знайдено 6 подій')]"
    ...
    assert events_find.is_displayed(), "Знайдено 6 подій"
    ...
    events_reset_xpath = "//p[contains(text(), 'Знайдено 47 подій')]"
    wait.until(lambda d: d.find_element(By.XPATH, events_reset_xpath).is_displayed())
    events_reset = driver.find_element(By.XPATH, events_reset_xpath)
    assert events_reset.is_displayed(), "Знайдено 47 подій"

    print(events_reset.text)
